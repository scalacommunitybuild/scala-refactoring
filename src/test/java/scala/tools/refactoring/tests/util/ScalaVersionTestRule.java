...
public class ScalaVersionTestRule implements MethodRule {
  ...
  public Statement apply(Statement stmt, FrameworkMethod meth, Object arg2) {
    ...
    String versionString = Properties.versionNumberString();

    if (onlyOn != null) {
      if (!onlyOn.doesNotMatch().isEmpty() && versionString.startsWith(onlyOn.doesNotMatch())) {
        return new EmptyStatement();
      } else if (versionString.startsWith(onlyOn.matches())) {
        return stmt;
      } else {
        return new EmptyStatement();
      }
